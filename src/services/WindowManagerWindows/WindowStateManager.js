    ...
    processWindows(rawWindows) {
        try {
            if (!rawWindows || !Array.isArray(rawWindows)) {
                console.warn('WindowStateManager: Invalid windows data provided', rawWindows);
                return [];
            }

            // Log pour debug
            console.log(`WindowStateManager: Processing ${rawWindows.length} raw windows:`,
                rawWindows.map(w => w.Title || 'Unknown').join(', '));
            ...
            // Log pour debug
            console.log(`WindowStateManager: Processed into ${processedWindows.length} windows:`,
                processedWindows.map(w => w.character || 'Unknown').join(', '));
            ...
            console.error('Error in processWindows:', error);
            this.errorHandler.error(error, 'WindowStateManager.processWindows');
            return rawWindows || []; // Return original windows as fallback
        }
    }
